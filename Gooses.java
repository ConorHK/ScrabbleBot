...
public class Gooses implements BotAPI {
	...
	private static class BoardExtended extends Board {
		...
		private boolean isValidAnchor(int row, int column) {
			if (!this.board.getSquareCopy(row, column).isOccupied()) {
				...
				return this.board.getSquareCopy(boxTop, column).isOccupied()
						|| this.board.getSquareCopy(boxBottom, column).isOccupied()
						|| this.board.getSquareCopy(row, boxLeft).isOccupied()
						|| this.board.getSquareCopy(row, boxRight).isOccupied();
			}
			return false;
		}
		...
		private void getHorizontalCrossSet(int row, int column, Node root, BoardAPI board,
				BoardExtended boardExtended) {
			Node word = root;
			// gets prefix hook
			if (board.getSquareCopy(row - 1, column).isOccupied()
					&& board.getSquareCopy(row + 1, column).isOccupied()) { // if there is a tile left & right of
																			// position
				...
				int adjacentRow = row + 1;
				while (board.getSquareCopy(adjacentRow + 1, column).isOccupied()) { // go to end of occupied right-side
																					// tiles
					adjacentRow++;
				}
	...
	private static class GADDAG {
		...
		public static Node build(Scanner words) {
			...
			while (words.hasNext()) {
				...
				String reversedWordWithSeparator = reverse(word.substring(0, word.length() - 1)) + "$"
		...
		public void findRowMoves(int offset, int anchorRow, int anchorColumn, Move workingMove,
				ArrayList<Character> frame, Node currNode, BoardAPI board, List<Move> moves, boolean reverse,
				BoardExtended boardExtended) {
			if (anchorRow + offset >= Board.BOARD_SIZE || anchorRow + offset < 0) {
				return;
			}

			if (board.getSquareCopy(anchorRow + offset, anchorColumn).isOccupied()) { // include any letters on left of
																						// anchor
				...
				findRowMovesNextPosition(offset, anchorRow, anchorColumn, letter, newMove, frame, currNode, nextNode,
						board, moves, reverse, boardExtended); // move on to the next square
			} else if (!frame.isEmpty()) {
				for (Character c : frame) {
					if (c != Tile.BLANK
							&& boardExtended.getSquareExtended(anchorRow + offset, anchorColumn).legalVertical(c)) { // if
																														// not
																														// blank
																														// +
																														// is
						...
						findRowMovesNextPosition(offset, anchorRow, anchorColumn, c, newMove, newRack, currNode,
								nextNode, board, moves, reverse, boardExtended);
					}
				}
			}
		}

		public void findRowMovesNextPosition(int offset, int anchorRow, int anchorColumn, char letter, Move workingMove,
				ArrayList<Character> frame, Node currNode, Node nextNode, BoardAPI board, List<Move> moves,
				boolean reverse, BoardExtended boardExtended) {
			if (offset <= 0) { // if making prefix
				if (currNode.isEnd(letter) && !board.getSquareCopy(anchorRow + offset - 1, anchorColumn).isOccupied()
						&& !board.getSquareCopy(anchorRow + 1, anchorColumn).isOccupied()) { // if its a valid move
					if (frame.isEmpty())
				...
				if (nextNode != null) { // generate next prefix
					findRowMoves(offset - 1, anchorRow, anchorColumn, workingMove, frame, nextNode, board, moves,
							reverse, boardExtended);

					nextNode = nextNode.getChild('$');
					if (nextNode != null && !board.getSquareCopy(anchorRow + offset - 1, anchorColumn).isOccupied()) { // generate
																														// suffixes
						findRowMoves(1, anchorRow, anchorColumn, workingMove, frame, nextNode, board, moves, false,
								boardExtended);
					}
				}
			} else if (offset > 0) {// if making suffix
				if (currNode.isEnd(letter) && !board.getSquareCopy(anchorRow + offset + 1, anchorColumn).isOccupied()) { // if
																															// its
																															// a
																															// valid
																															// move
				...
				if (nextNode != null && !board.getSquareCopy(anchorRow + offset + 1, anchorColumn).isOccupied()) { // generate
																													// suffixes
					currNode = nextNode;
					findRowMoves(offset + 1, anchorRow, anchorColumn, workingMove, frame, currNode, board, moves,
							reverse, boardExtended);
				}
			}
		}

		public void findColumnMoves(int offset, int anchorRow, int anchorColumn, Move workingMove,
				ArrayList<Character> rack, Node currNode, BoardAPI board, List<Move> moves, boolean reverse,
				BoardExtended boardExtended) {
			if (anchorColumn + offset >= Board.BOARD_SIZE || anchorColumn + offset < 0) {
				return;
			}

			if (board.getSquareCopy(anchorRow, anchorColumn + offset).isOccupied()) { // include any letters above
																						// anchor
				...
				findColumnMovesNextPosition(offset, anchorRow, anchorColumn, l, newMove, rack, currNode, nextNode,
						board, moves, reverse, boardExtended); // move on to the next square
			} else if (!rack.isEmpty()) {
				for (Character c : rack) {
					if (c != Tile.BLANK
							&& boardExtended.getSquareExtended(anchorRow, anchorColumn + offset).legalHorizontal(c)) { // if
																														// not
																														// blank
																														// +
																														// is
						...
						findColumnMovesNextPosition(offset, anchorRow, anchorColumn, c, newMove, newRack, currNode,
								nextNode, board, moves, reverse, boardExtended);
					}
				}
			}
		...
		public void findColumnMovesNextPosition(int offset, int anchorRow, int anchorColumn, char letter,
				Move workingMove, ArrayList<Character> rack, Node currNode, Node nextNode, BoardAPI board,
				List<Move> moves, boolean reverse, BoardExtended boardExtended) {
			if (offset <= 0) { // if making prefix
				if (currNode.isEnd(letter) && !board.getSquareCopy(anchorRow + offset - 1, anchorColumn).isOccupied()
						&& !board.getSquareCopy(anchorRow, anchorColumn + 1).isOccupied()) {
					if (rack.isEmpty())
						workingMove.usesAllTiles = true;
					moves.add(workingMove);
				}
				if (nextNode != null) { // generate prefixes
					findColumnMoves(offset - 1, anchorRow, anchorColumn, workingMove, rack, nextNode, board, moves,
							reverse, boardExtended);
					nextNode = nextNode.getChild('$');
					if (nextNode != null && !board.getSquareCopy(anchorRow, anchorColumn + offset - 1).isOccupied()) { // generate
																														// suffixes
						findColumnMoves(1, anchorRow, anchorColumn, workingMove, rack, nextNode, board, moves, false,
								boardExtended);
					}
				}
				...
				if (nextNode != null && !board.getSquareCopy(anchorRow, anchorColumn + offset + 1).isOccupied()) { // generate
																													// suffixes
					currNode = nextNode;
					findColumnMoves(offset + 1, anchorRow, anchorColumn, workingMove, rack, currNode, board, moves,
							reverse, boardExtended);
				}
			}
		}
	...
	public void testGADDAG(String frame, BoardExtended boardExtended) {
		...
		// System.out.println("Start row: " + highestScoringMove.places.get(0).getRow()
		// + " Start column: " + highestScoringMove.places.get(0).getColumn());
	...
	private int calculateScore(Move move, BoardExtended boardExtended, BoardAPI board) {
		...
		for (Place place : move) {
			...
			if (boardExtended.isAnchor(place.getRow(), place.getColumn())) {
				...
				connectingWordsScore += placement.getValue()
						* board.getSquareCopy(place.getRow(), place.getColumn()).getLetterMuliplier();

				if (connectingWords) {
					connectingWordsScore = connectingWordsScore
							* board.getSquareCopy(place.getRow(), place.getColumn()).getWordMultiplier();
